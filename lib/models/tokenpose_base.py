...
class Transformer(nn.Module):
    ...
    def forward(self, x, mask = None,pos=None):
        #for the functions attn and ff in each layer, i run x (passed in) through the attn() and ff() 
        for idx,(attn, ff) in enumerate(self.layers): #?what is ff, does it match to Residual(PreNorm(dim, FeedForward(dim, mlp_dim, dropout = dropout))
            if idx>0 and self.all_attn: #false & if statement doesn't run
...
class TokenPose_L_base_TAF(nn.Module):
    def __init__(self, *, feature_size, patch_size, num_keypoints, dim, depth, heads, mlp_dim, apply_init=False, hidden_heatmap_dim=64*6,heatmap_dim=64*48,taf_heatmap_dim=64*48*2, heatmap_size=[64,48,2], channels = 3, dropout = 0., emb_dropout = 0.,pos_embedding_type="learnable", taf_dim=64*48*2, taf_size=[64,48,2]):
        super().__init__()
        assert isinstance(feature_size,list) and isinstance(patch_size,list), 'image_size and patch_size should be list'
        assert feature_size[0] % patch_size[0] == 0 and feature_size[1] % patch_size[1] == 0, 'Image dimensions must be divisible by the patch size.'
        num_patches = (feature_size[0] // (patch_size[0])) * (feature_size[1] // (patch_size[1]))
        ...
        #taf_dim =  #taf_dim passed in maybe: b, time, vectors --> currently images individually processed --> don't know how to process images at near time frames w each other, change entire dimensions?

      # Temporal affinity mlp models
        '''
        self.mlp_head2 = nn.Sequential(
            nn.LayerNorm(dim*3),
            nn.Linear(dim*3, hidden_heatmap_dim),
            nn.LayerNorm(hidden_heatmap_dim),
            nn.Linear(hidden_heatmap_dim, taf_dim) #taf_dim currently undefined once we know what the dimension is & need to define taf dimensions
        )
        '''
        self.mlp_head2 = nn.Sequential(
            nn.LayerNorm(dim*3),
            nn.Linear(dim*3, hidden_heatmap_dim),
            nn.LayerNorm(hidden_heatmap_dim),
            nn.Linear(hidden_heatmap_dim, taf_heatmap_dim)
        )
    ...
    def forward(self, feature, mask = None):
        ...
        x = torch.cat((x1_out, x2_out, x3_out), dim=2)
        x_2 = torch.cat((x1_out, x2_out, x3_out), dim=2) #created a copy of the output to go into taf

        x = self.mlp_head(x)
        x = rearrange(x,'b c (p1 p2) -> b c p1 p2',p1=self.heatmap_size[0],p2=self.heatmap_size[1])

        print('x_2.shape', x_2.shape)
        taf = self.mlp_head2(x_2) #taf results not returned yet
        print('taf.shape', taf.shape)
        
        taf = rearrange(taf,'b c (p1 p2 p3) -> b c p1 p2 p3',p1=self.heatmap_size[0],p2=self.heatmap_size[1],p3=self.heatmap_size[2])
        #print('taf type is ', type(taf)) #self.heatmap_size[0]
        print('taf after .shape', taf.shape)
